...
\begin{abstract}
        ...
        \paragraph*{Background:} High-throughput genotyping microarrays can be used not only to assess changes in total DNA copy number but also changes in allele-specific copy numbers (ASCNs). Even after state of the art preprocessing methods, Affymetrix genotyping arrays still suffer from systematic effects that make them difficult to use effective for downstream ASCN studies in cancers.
...
%  $TCN = 2~\frac{\theta}{\theta^R}$ and the fraction of allele B $\beta = \frac{\theta_B}{\theta}$ are often reported, where $\theta = \theta_A + \theta_B$ is the total intensity for the SNP and $\theta^R$ is the total intensity for the same SNP for a reference, which is typically either a robust average of a large set of arrays or a paired normal.
