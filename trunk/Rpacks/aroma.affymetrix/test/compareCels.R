...
# @keyword "IO"
#*/#########################################################################
compareCels <- function(pathname, other, verbose=0, ...) {
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  # Local functions
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  ...
  if (verbose)
    cat("Reading first...\n");
  cel1 <- readCel(pathname, readHeader=TRUE, readIntensities=TRUE, readStdvs=TRUE, readPixels=TRUE, readOutliers=FALSE, readMasked=FALSE);
  if (verbose)
    cat("Reading first...done\n");

  if (verbose)
    cat("Reading second...\n");
    cel2 <- readCel(other, readHeader=TRUE, readIntensities=TRUE, readStdvs=TRUE, readPixels=TRUE, readOutliers=FALSE, readMasked=FALSE);
  if (verbose)
  ...
  for (ff in setdiff(names(cel$header), excl)) {
    h1 <- cel1$header[[field]];
    h2 <- cel2$header[[field]];
    if (!identical(h1, h2)) {
  ...
  for (field in c("intensities", "stdvs", "pixels")) {
    v1 <- cel1[[field]];
    v2 <- cel2[[field]];
str(v1)
str(v2)
    if (!identical(all.equal(v1, v2), TRUE)) {
      throw("Validation of new CEL file failed. Field differ: ", field);
    }
  }
  if (verbose >= 1)
