...
if (!exists("sets", mode="list")) {
  sets <- list(
   "ACC,+300,RMA,A+B,FLN,-X" = list(col=niceCols[1], name="ACC+FLN"),
   "ACC,+300,RMA,A+B" = list(col=niceCols[2], name="ACC"),
   "QN,RMA,A+B" = list(col=niceCols[3], name="QN")
  );
}
...
if (!Device$isOpen(fig <- fig + 1)) {
  ...
  if (d < 0.20) {
    xs <- c(0.0535, 0.06, 0.0645);
    ys <- c(0.945, 0.937, 0.93);
    srt <- rep(51, 3);
    adjy <- c(-0.3,-0.3,-0.3);
