...
setMethodS3("convertToUnique", "AffymetrixCelSet", function(this, ..., tags="UNQ", force=FALSE, verbose=FALSE) {
  ...
  outputDataSet<-try( AffymetrixCelSet$fromName(getName(this),tags=tags,verbose=verbose,
                      cdf=cdfUnique,chipType=getChipType(this),checkChipType=FALSE), silent=TRUE )
  
  if (class(outputDataSet)[1]=="AffymetrixCelSet") {
  ...
  cdfStandard <- readCdf(getPathname(cdf), units=NULL,readXY=FALSE, readBases=FALSE, 
  ...
  cdfUniqueIndices <- readCdf(getPathname(cdfUnique), units=NULL,readXY=FALSE, readBases=FALSE, 
              readIndexpos=FALSE, readAtoms=FALSE,readUnitType=FALSE, readUnitDirection=FALSE, 
              readUnitNumber=FALSE, readUnitAtomNumbers=FALSE, readGroupAtomNumbers=FALSE, readGroupDirection=FALSE, 
              readIndices=TRUE, readIsPm=FALSE)
  verbose && exit(verbose);
  ...
  for (kk in seq_len(nbrOfArrays)) {
      ...
      header <- list(filename=pathname, version=4, rows = nr, cols = nc, total=nr*nc, 
	                 noutliers=0, chiptype=getChipType(cdfUnique),header=hdr$header,
					 algorithm=hdr$algorithm, parameters=hdr$parameters, cellmargin=hdr$cellmargin,
					 nmasked=hdr$nmasked )
