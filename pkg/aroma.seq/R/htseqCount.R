...
#   \item{gf}{gene feature file, in gff format}
...
#
#*/###########################################################################
setMethodS3("htseqCount", "default", function(samFile,
                                              gf,
                                              outFile=NULL,
  ...
  ## ( Support a call like this: "htseq-count -s no -a 10 lib_sn.sam gf > countFile")
  ...
  # Argument 'gf'
  if (!is.null(gf)) {
    gf <- Arguments$getReadablePathname(gf);
  } else {
    throw("Argument gf is empty; supply gene ('transcript') model file")
  }
