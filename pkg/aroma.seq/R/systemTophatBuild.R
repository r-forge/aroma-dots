...
setMethodS3("systemTopHatBuild", "default", function( ## ( No 'command' arg in this case )
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  # Split up '...' arguments by system2() and TopHat executable
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  keys <- names(args);
  if (is.null(keys)) {
    system2Args <- NULL;
  ...
  if (FALSE) {
    ...
    keys <- names(args);
    keys <- sprintf("-%s", keys);  ## Just add a dash to everything!
  } else {
    keys <- keys  ## I.e. just pass along as is!
  }
