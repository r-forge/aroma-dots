...
if (!exists("fracBDsList", mode="list")) {
  ...
  pattern <- sprintf("^%s(|,TBN.*|,CMTN)$", dataSet);
  fracBDsList <- loadAllDataSets(dataSet, chipType=chipType, pattern=pattern, type="fracB", rootPath=rootPath, verbose=verbose);
  rm(pattern);
...
keep <- sapply(names, FUN=function(tags) {
  is.element("raw", tags) ||   is.element("CMTN", tags) || any(is.element(genTags, tags))
});
...
figForce <- 3;
figDev <- function(..., force=(figForce > 0), scale=1) { epsDev(..., path=figPath, force=force) }
