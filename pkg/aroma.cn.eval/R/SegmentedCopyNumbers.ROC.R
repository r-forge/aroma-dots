...
#
# \seealso{
#   @see "aroma.core::SegmentedCopyNumbers"
# }
#*/###########################################################################
setMethodS3("fitRoc", "SegmentedCopyNumbers", function(this, states=NULL, recall=states[1], ...) {
  # Extract by state?
  if (!is.null(states)) {
...
#
# @title "Estimates the true-positive rate at a given false-positive rate for calling copy numbers of state"
#
# \description{
#  @get "title".
# }
...
# }
#*/###########################################################################
setMethodS3("findRocTpAtFp", "SegmentedCopyNumbers", function(this, states=NULL, recall=states[1], ...) {
  # Extract by state?
  if (!is.null(states)) {
    this <- extractSubsetByState(this, states=states);
  }
...
# }
#*/###########################################################################
setMethodS3("findRocSmoothingForTpAtFp", "SegmentedCopyNumbers", function(this, minTpRate=0.95, fpRate=0.05, states=NULL, recall=states[1], nstepsR=2, accTp=0.001, accR=0.01, ..., verbose=FALSE) {
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  # Validate arguments
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  # Argument 'minTpRate':
  ...
  verbose && enter(verbose, "Extracting loci with CN states of interest");
  cn <- extractSubsetByState(this, states=states, recall=recall);
  ...
  while (TRUE) {
    ...
    fit <- findRocTpAtFp(cnS, fpRate=fpRate, recall=recall, ..., verbose=less(verbose));
